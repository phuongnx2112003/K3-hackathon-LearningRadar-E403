...
const AI_SERVICE_TIMEOUT_MS = Number(process.env.AI_SERVICE_TIMEOUT_MS) || 40000;
...
async function postToAiService(path, payload) {
  ...
  try {
    ...
    if (error.name === "AbortError") throw new Error(`AI service timed out after ${AI_SERVICE_TIMEOUT_MS}ms`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(errorBody.error?.message || `AI service responded with ${response.status}`);
  }
...
async function askTutor(payload) {
  return postToAiService("/ai/ask", payload);
}
