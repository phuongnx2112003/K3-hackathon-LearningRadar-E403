...
const Sidebar = (props) => {
  ...
  return (
    <nav className={cn(s.root, {[s.sidebarOpen]: burgerBtnToggled})}>
      ...
      <ul className={s.nav}>
        <h5 className={[s.navTitle, s.groupTitle].join(" ")}>VLEARN AI RADAR</h5>
        <LinksGroup
          onActiveSidebarItemChange={activeItem => props.dispatch(changeActiveSidebarItem(activeItem))}
          activeItem={props.activeItem}
          header="Student Reader"
          isHeader
          iconName={<i className="eva eva-book-open-outline"/>}
          link="/template/vlearn/student"
          index="vlearn-student"
        />
        <LinksGroup
          onActiveSidebarItemChange={activeItem => props.dispatch(changeActiveSidebarItem(activeItem))}
          activeItem={props.activeItem}
          header="Teacher Radar"
          isHeader
          iconName={<i className="eva eva-pie-chart-outline"/>}
          link="/template/vlearn/teacher"
          index="vlearn-teacher"
        />
        <LinksGroup
          onActiveSidebarItemChange={activeItem => props.dispatch(changeActiveSidebarItem(activeItem))}
          activeItem={props.activeItem}
          header="Eval Suite"
          isHeader
          iconName={<i className="eva eva-options-2-outline"/>}
          link="/template/vlearn/eval"
          index="vlearn-eval"
        />
        <LinksGroup
          onActiveSidebarItemChange={activeItem => props.dispatch(changeActiveSidebarItem(activeItem))}
          activeItem={props.activeItem}
          header="Dashboard Mẫu"
          isHeader
